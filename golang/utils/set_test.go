...
func TestSetBasic(t *testing.T) {
	...
	assert.Equal(t, len(s), 0, "Set contained elements")

	s.AddMany([]string{"foo", "bar", "baz"})
	assert.Equal(t, len(s), 3, "Set did not contain all added elements")
	assert.True(t, s.Contains("foo"), "Set did not contain added element")
...
func TestIntersection(t *testing.T) {
	s1 := NewSet("").AddMany([]string{"foo", "bar", "wibble"})
	s2 := NewSet("").AddMany([]string{"bar", "baz", "wibble"})
...
func TestIsSubSet(t *testing.T) {
	s1 := NewSet("").AddMany([]string{"bar", "baz"})
	s2 := NewSet("").AddMany([]string{"foo", "bar", "baz"})
